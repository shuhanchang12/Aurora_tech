...
export default function PresentationViewer({ 
    ...
    const colorConfig = {
        indigo: { bg: 'bg-indigo-900', border: 'border-indigo-500', text: 'text-indigo-400', active: 'bg-indigo-600' },
        blue: { bg: 'bg-slate-900', border: 'border-blue-500', text: 'text-blue-400', active: 'bg-blue-600' },
        rose: { bg: 'bg-slate-900', border: 'border-rose-500', text: 'text-rose-400', active: 'bg-rose-600' },
        emerald: { bg: 'bg-slate-900', border: 'border-emerald-500', text: 'text-emerald-400', active: 'bg-emerald-600' },
    };
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-slate-200">
                ...
                <div className="flex-1 flex justify-center items-center p-8 relative transition-all overflow-hidden" style={{ backgroundImage: 'radial-gradient(circle at center, #cbd5e1 1px, transparent 1px)', backgroundSize: '24px 24px' }}>
                   <div className={`w-full aspect-[16/9] max-w-4xl max-h-full flex flex-col border border-slate-300 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] bg-slate-900 text-white overflow-hidden relative transition-transform duration-300`}>
                        ...
                        <div className="p-8 md:p-12 flex flex-col h-full z-10 w-full relative">
                            ...
                            {slides[currentSlide].visualComponent ? (
                                <div className="flex-1 flex flex-col items-center justify-center">
                                    <div className={`text-${accentColor}-400 mb-6 bg-slate-800/80 p-8 rounded-2xl border border-slate-700 shadow-lg w-full max-w-lg flex flex-col items-center justify-center`}>
                                        {slides[currentSlide].visualComponent}
                                    </div>
                        ...
                        <div className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-transparent to-${accentColor}-900/20 rounded-bl-full pointer-events-none`}></div>
                   </div>
                </div>
